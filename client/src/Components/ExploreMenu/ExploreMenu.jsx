...
const ExploreMenu = ({category,setCategory}) => {
  return (
    <div className="explore_menu flex flex-col gap-4 " id="explore_menu">
      ...
      <p className="max-w-3/5 text-[#262626] font-semibold ">
        Discover a variety of mouthwatering dishes crafted with the freshest
        ingredients and bursting with flavor. Indulge in a culinary experience
        like no other!
      </p>
      <div className="menu_list flex justify-between items-center text-center p-4 ">
        {menu_list.map((item, index) => (
          ...
          >
            <img
              src={item.menu_image}
              className={category === item.menu_name ? "active w-24 " : "w-24 "}
              alt=""
            />
